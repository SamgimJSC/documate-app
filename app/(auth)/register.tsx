...
export default function RegisterScreen() {
  ...
  const validate = () => {
    ...
    if (!email || !/\S+@\S+\.\S+/.test(email)) e.email = '올바른 이메일을 입력해주세요.';
    if (!password || password.length < 8) e.password = '비밀번호는 8자 이상이어야 합니다.';
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={styles.flex}>
        <ScrollView contentContainerStyle={styles.scroll} keyboardShouldPersistTaps="handled">
          ...
          <View style={styles.card}>
            ...
            <Text style={styles.subtitle}>계정을 만들어 스마트한 문서 관리를 경험하세요</Text>
            ...
            <Button label="회원가입" onPress={handleRegister} loading={loading} />
